...
print('seg_df shape:', seg_df.shape)  # 87, 2
seg_df.head(3)
...
for file_name in tqdm(study_uid_list):
    # Every File, read out the mask and image
    ...
    assert image.shape == mask.shape, f"Image and mask {file_name} should be the same size, " \
                                      f"but are {image.shape} and {mask.shape}"

    slice_num = image.shape[2]

    for i in range(1, slice_num - 1):
        image_25d = image[:, :, i - 1:i + 2]   # Slide windows 3 mimic [RGB]
        mask_25d = mask[:, :, i - 1:i + 2]
        assert image_25d.shape == mask_25d.shape == (512, 512, 3), \
            f"Image and mask {file_name} should be (512, 512, 3), but are {image_25d.shape} and {mask_25d.shape}"
        ...
        # slice_num is the center frame...



seg_25d_df = pd.DataFrame(dataframe_list,
                          columns=["id", "StudyInstanceUID", "slice_num", "image_path", "mask_path"])
