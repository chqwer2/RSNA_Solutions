...
from dataset import get_transforms_16bit_light, get_transforms_16bit_heavy
...
class CFG:
    ...
    pl_path = f'./input/vindr/train_predictions.csv'

    checkpoint_path = './input/checkpoint/0001_model_f0_ep8.pth'
...
pl_images_dir = f'./input/images_gpugen/vindr_1536896_16bit_cutoff'
...
df_train_pl = pd.read_csv(CFG.pl_path)

# # hard label
# df_train_pl = df_train_pl[df_train_pl["breast_birads"] != 4].reset_index(drop=True)
# df_train_pl["cancer"] = df_train_pl["breast_birads"].apply(lambda x:x==5).astype(int)
# df_train_pl.loc[df_train_pl["cancer"]==1, "sample_rand"] = 0.0

# soft label
df_train_pl["cancer"] = df_train_pl["cancer_pred_proba"]


CFG.temperature = 1.0  # 0~1
def cook_labels(y):
    mask = y < 0.5
...
_ = cook_labels(df_train_pl["cancer"])

df_train_pl.loc[df_train_pl["cancer"]>=0.6, "sample_rand"] = 0.0
...
# %%
if DEBUG:
    ds_train = BreastCancerDataSet_16bit(df_train_pl, pl_images_dir, CFG.target, get_transforms_16bit_heavy('train', CFG.image_size, CFG.normalize_mean, CFG.normalize_std))
    X, y_cancer = ds_train[42]
    print(f"X.shape: {X.shape}, y_cancer.shape: {y_cancer.shape}")

dataset_show = BreastCancerDataSet_16bit(df_train_pl, pl_images_dir, CFG.target, get_transforms_16bit_heavy('train', CFG.image_size, CFG.normalize_mean, CFG.normalize_std))
for i in range(2):
...
def valid_one_epoch(model, dataloader):
    ...
    with torch.no_grad():
        model.eval()
        losses = []; targets = []
        with tqdm(dataloader, desc='Eval', mininterval=10) as progress:
...
def train_loop(logger, fold, do_save_model=True):
    ...
    ds_train = BreastCancerDataSet_16bit(tr_df, pl_images_dir, CFG.target, get_transforms_16bit_heavy('train', CFG.image_size, CFG.normalize_mean, CFG.normalize_std))
    ds_valid = BreastCancerDataSet_16bit(va_df, images_dir, CFG.target, get_transforms_16bit_heavy('valid', CFG.image_size, CFG.normalize_mean, CFG.normalize_std))
    ...
    for epoch in range(CFG.epochs):
        ...
        if f1 > best_valid_score:
            ...
            if do_save_model and epoch > 0:
                save_name = f'{output_dir}/{CFG.suff}_{CFG.model_arch}_PL_f{fold}_ep{epoch}.pth'
...
def gen_predictions(models, df_train, folds):
    ...
    for model, fold in zip(models, folds):
        ds_valid = BreastCancerDataSet_16bit(df_train.query('split == @fold'), images_dir, CFG.target, get_transforms_16bit_heavy('valid', CFG.image_size, CFG.normalize_mean, CFG.normalize_std))
